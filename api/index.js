...
var app = new Elysia().use(html()).use(staticPlugin()).use(
  jwt({
    name: "jwt",
  ...
  return { profile: null };
}).get("/", () => Bun.file("src/public.html")).post(
  "/activate",
  async ({ body, set }) => {
    ...
    if (keyToActivate && keyToActivate.status === "UNUSED") {
      await prisma.licenseKey.update({
        where: { serialKey },
        data: { status: "ACTIVATED", deviceId, activatedAt: /* @__PURE__ */ new Date() }
      });
  ...
  "/admin",
  (app2) => app2.get("/", () => Bun.file("src/admin.html")).post(
    "/login",
    async ({ jwt: jwt2, body, set }) => {
    ...
    async ({ body, set, profile }) => {
      ...
      try {
        ...
        console.log(
          `Admin '${profile.username}' generated new key for ${body.email}`
        );
      } catch (error) {
        if (error.code === "P2002") {
          set.status = 409;
...
app.listen(3e3, ({ hostname, port }) => {
  console.log(`\u{1F680} Licensing server running at http://${hostname}:${port}`);
  console.log(
    `\u{1F511} Admin dashboard available at http://${hostname}:${port}/admin`
  );
});
